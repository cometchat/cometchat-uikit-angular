...
class Renderer extends EventEmitter<RendererEvents> {
  ...
  private parentFromOptionsContainer(
    ...
    if (typeof container === 'string') {
      parent = this.iframeDocument.querySelector(
        container
      ...
      this.iframeWindow &&
      (container as any) instanceof
        (this.iframeWindow as Window & typeof globalThis)?.HTMLElement
    ) {
  ...
  private convertColorValues(
    ...
    const gradientHeight =
      canvasElement.height * (this.iframeWindow.devicePixelRatio || 1);
  ...
  private renderMultiCanvas(
    ...
    let singleCanvasWidth = Math.min(
      Renderer.MAX_CANVAS_WIDTH,
      clientWidth,
      totalWidth
    );
    ...
    if (options.barWidth || options.barGap) {
      ...
      if (singleCanvasWidth % totalBarWidth !== 0) {
        singleCanvasWidth =
          Math.floor(singleCanvasWidth / totalBarWidth) * totalBarWidth;
      }
    }
    ...
    const draw = (index: number) => {
      ...
      const data = channelData.map((channel) => {
        ...
        const end = Math.floor(
          ((offset + clampedWidth) / totalWidth) * channel.length
        );
    ...
    if (numCanvases > 1) {
      this.unsubscribeOnScroll = this.on('scroll', () => {
        ...
        const canvasIndex = Math.floor(
          (scrollLeft / totalWidth) * numCanvases
        );
        clearCanvases();
        draw(canvasIndex - 1);
        draw(canvasIndex);
  ...
  async render(audioData: AudioBuffer) {
    ...
    const scrollWidth = Math.ceil(
      audioData.duration * (this.options.minPxPerSec || 0)
    );
  ...
  async exportImage(
    ...
    // Data URLs
    if (type === 'dataURL') {
      const images = Array.from(canvases).map((canvas) =>

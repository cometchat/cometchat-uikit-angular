...
| `callSettingsBuilder` | `SessionSettings` | `undefined` | Custom [v5 SessionSettings object](https://www.cometchat.com/docs/calls/javascript/session-settings) to override the default call settings used in the ongoing call overlay. Follows the three-tier priority: @Input > [GlobalConfig](/customization/global-config) > default. |
...
Override the default session settings used in the ongoing call overlay when a call is initiated from the call logs:

...
| `callSettingsBuilder` | `SessionSettings` | `undefined` | Custom [v5 SessionSettings object](https://www.cometchat.com/docs/calls/javascript/session-settings) to override the default call settings used in the ongoing call screen. Follows the three-tier priority: @Input > [GlobalConfig](/customization/global-config) > default. |
...
| `hideOverlays` | `boolean` | `false` | When `true`, hides the incoming/outgoing call overlays managed by this component. Useful when you handle call UI externally. |
...
Override the default session settings to customize the call UI:

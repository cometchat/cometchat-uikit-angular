...
| `parentMessage` | `CometChat.BaseMessage` | `undefined` | The thread's parent message. When set, the component shows only replies to it. Preferred over `parentMessageId` — the scoping id is derived from this, and the object also carries the thread's subscription state, which a reply arriving in real time has no flag of its own to read. |
| `parentMessageId` | `number` | `undefined` | **Deprecated — pass `parentMessage` instead.** Still honoured when `parentMessage` is absent, so existing callers keep working; they just opt out of realtime thread-subscription state on replies. |
...
| `hideFlagMessageOption` | `boolean` | `false` | Hides the "Flag/Report" option from message context menu. |
| `hideFlagRemarkField` | `boolean` | `false` | Hides the optional remark text area in the flag message dialog. |
| `showMarkAsUnreadOption` | `boolean` | `true` | Shows the "Mark as Unread" option in the message context menu. Set to `false` to hide it. |
| `hideReplyOption` | `boolean` | `false` | Hides the "Reply" option from message context menu. |
| `startFromUnreadMessages` | `boolean` | `true` | When `true`, the message list scrolls to the first unread message on load instead of the bottom. |
| `isAgentChat` | `boolean` | `false` | When `true`, configures the message list for AI agent chat mode. Accepts messages with `parentMessageId` without filtering, enabling threaded AI conversations. |

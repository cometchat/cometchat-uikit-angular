...
| `callSettingsBuilder` | `SessionSettings` | `undefined` | Custom [v5 SessionSettings object](https://www.cometchat.com/docs/calls/javascript/session-settings) for call components (call buttons, call logs, ongoing call) |
...
The `callSettingsBuilder` property lets you provide a custom v5 `SessionSettings` object that is used across all call-related components. This is useful for configuring call UI features like layout, recording, screen sharing, or audio-only mode globally.
...
// v5: pass a plain SessionSettings object instead of using CallSettingsBuilder
const customCallSettings = {
  sessionType: 'VIDEO',
  layout: 'TILE',
...
// v5: plain SessionSettings object — no CallSettingsBuilder needed
const callSettings = {
  sessionType: 'VIDEO',
  layout: 'TILE',
  startAudioMuted: false,
